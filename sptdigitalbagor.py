...
def create_docx_from_template(data, signature_img):
    ...
    try:
        ...
        # MAPPING KUNCI (KEY) HARUS SAMA DENGAN {{TAG}} DI WORD
        context = {
...
list_opd = ["Bagian Organisasi", "Bagian Umum", "Bagian Tata Pemerintahan", "Dinas Pendidikan dan Kebudayaan", "Dinas Kesehatan", "RSUD Ahmad Ripin"]
...
with st.form("spt_form"):
    ...
    with col2:
        pangkat_admin = st.text_input("Pangkat")
...
if submit_button:
    if not unit_kerja_final or not nama_admin:
        st.warning("⚠️ Mohon isi Unit Kerja dan Nama Admin.")
    else:
        try:
            with st.spinner('Menghasilkan dokumen...'):
                ...
                if docx_file:
                    ...
                    st.download_button(label="📥 Download SPT (Word)", data=docx_file, file_name=f"SPT_{nama_admin}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
        except Exception as e:
            st.error(f"Error: {e}")
